from neuwon import Model, builder
import neuwon.mechanisms.hh as hh
import numpy as np
import bisect

class AP_Experiment:
    """ A very simple model with HH channels to experiment with. """
    def __init__(self,
            axon_length   = 1000e-6,
            axon_diameter = 1e-6,
            soma_diameter = 20e-6,
    ...
    def make_model(self):
        ...
        self.builder = builder.ModelBuilder()
        self.soma = [self.builder.add_neuron([0,0,0], self.soma_diameter)]
        ...
        for x in self.axon:
            ...
            x.insert_mechanism(hh.VoltageGatedSodiumChannel)
            x.insert_mechanism(hh.VoltageGatedPotassiumChannel)
        self.model = self.builder.build_model(self.time_step, stagger=self.stagger)
        ...
        # print("Average Electric Connectivity: ", self.model.electric_irm_density() * len(self.model))
        # print()
    ...
    def run_experiment(self):
        ...
        for t, inp in enumerate(self.input_current):
            if inp is not None:
                self.model.inject_current(self.soma[0].index, inp)
            self.model.advance()
            self.time_stamps.append((t + 2) * self.time_step * 1e3)
            for idx, p in enumerate(self.probes):
                self.v[idx].append(self.model.electrics.voltages[p.index] * 1e3)
...
reduced, and that the remaining error is not proportional to the time step ∆t. """
    ...
    gold = AP_Experiment(time_step = 1e-6, **args)
    ...
    lockstep = AP_Experiment(time_step = 100e-6, stagger=False, **args)
    ...
    plt.subplot(2,2,2)
    lockstep_slow = AP_Experiment(time_step = 20e-6, stagger=False, **args)
    lockstep_fast = AP_Experiment(time_step = 10e-6, stagger=False, **args)
    ...
    plt.subplot(2,2,3)
    staggered = AP_Experiment(time_step = 100e-6, **args)
    plt.plot(gold.time_stamps, gold.v[0], 'k',
            label=make_label(gold))
    ...
    plt.subplot(2,2,4)
    staggered_slow = AP_Experiment(time_step = .1e-3, **args)
    staggered_fast = AP_Experiment(time_step = .05e-3, **args)
...
def analyze_length_step():
    x2 = AP_Experiment(length_step=10e-6)
    x3 = AP_Experiment(length_step=20e-6)
    x4 = AP_Experiment(length_step=100e-6)
    x5 = AP_Experiment(length_step=200e-6)
...
def analyze_axon_diameter():
    x1 = AP_Experiment(axon_diameter = .5e-6, stimulus=3e-9, time_step=2.5e-6)
    x2 = AP_Experiment(axon_diameter = 1e-6,  stimulus=3e-9, time_step=2.5e-6)
    x3 = AP_Experiment(axon_diameter = 2e-6,  stimulus=3e-9, time_step=2.5e-6)
    x4 = AP_Experiment(axon_diameter = 4e-6,  stimulus=3e-9, time_step=2.5e-6)
    x5 = AP_Experiment(axon_diameter = 8e-6,  stimulus=3e-9, time_step=2.5e-6)
...
the dynamics near that point."""
    x = AP_Experiment(probes=[0, .2, .4, .6, .8, 1.0], time_step=2.5e-6,)
